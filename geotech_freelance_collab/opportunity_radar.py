#!/usr/bin/env python3
"""Opportunity radar for an offshore/nearshore geotechnical freelancer.
...
TRIGGERS = {
    ...
    r"reclamation|polder|nearshore|coastal protection": 3,
}
# GEOGRAPHY he can reach on 14 days' notice, weighted by how likely he is hired there.
GEOGRAPHY = {
    r"singapore|malaysia|indonesia|vietnam|thailand|brunei|philippines|sarawak|sabah": 4,
...
def parse(xml_bytes, source):
    ...
    for it in root.iter("item"):  # RSS 2.0
        ...
        body = re.sub(r"<[^>]+>", " ", it.findtext("content:encoded", namespaces=ns) or "")
        date = it.findtext("pubDate") or ""
        items.append((source, title, link, f"{desc} {body}", date))
