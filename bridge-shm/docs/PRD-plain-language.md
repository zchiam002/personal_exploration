...
**What this is:** a companion to PRD v0.1 (`docs/PRD.md`) for readers without a vibration-engineering background. It says the same things in everyday language, and the glossary at the end explains the technical terms. Where the two differ, the original PRD is the reference. Placeholders in [brackets] are still to be filled in.
...
- Shake or hit the bridge deliberately to test it (EMA, impact testing).
- Tune a computer model of the bridge to match the measurements (FE model updating).
- Point to where damage is (damage localisation).
- Estimate how much load the bridge can carry from strain readings (load rating, planned for version 2).
- Manage several bridges at once.
- Offer a phone app.
...
**Getting the data in**
- FR1: The recorder produces one file (one "block" of data) every 30–60 minutes, sampling the accelerometers 100–200 times per second and the other sensors at their own rates, with the time stamped in UTC (universal time, so there is no time-zone confusion). Files are uploaded automatically on a schedule.
...
- FR4: Each block of data is cleaned first. Slow drifts are removed (detrend), the sample rate is reduced to what is needed (decimate), and frequencies outside the range of interest are filtered out (band-pass). The block is then checked for steady vibration from machinery or electrical interference (harmonics) that could be mistaken for the bridge.
- FR5: Two independent mathematical techniques extract the fingerprint from each block: SSI-cov as the main method and FDD as a cross-check (both explained in the glossary). Both come from pyOMA2, a published software toolkit whose licence terms for commercial use are still to be confirmed. The output for each note is a frequency, a damping value, a mode shape and an estimate of how confident to be (uncertainty bounds); the damping and confidence figures come mainly from SSI-cov, and FDD confirms the frequencies and shapes.
...
- FR7: Every block is tagged with the average temperature, a traffic index (a single number for how busy the bridge was) and strain statistics for that period.
- FR8: Temperature and traffic shift the fingerprint by themselves: a hot deck is slightly less stiff, and a queue of lorries adds mass. A statistical model (a regression or PCA, see glossary) learned during the baseline predicts this everyday shift. What is left after removing it (the residual) is what could indicate real change. Both the raw and the corrected values are stored.

**Showing the results**
- FR9: Dashboard. A timeline for each note, raw and corrected; how well today's shape still matches the reference (MAC); temperature, traffic and strain overlays; data-quality status; and whether the processing is running.
- FR10: Alerting. A control chart (a running chart with statistical limits) on the corrected values. The alarm levels (thresholds) are adjustable, alerts are logged and must be acknowledged, and the whole thing stays off until the baseline is signed off.
...
- Access to the owner's data is controlled: only people the owner has authorised (the owner's engineer, NTU and the platform operator) can see it. Whether to host on the owner's premises or in the cloud is still open.

## 6. What the data looks like
- Motion: [n] acceleration channels sampled 100–200 times per second, with a filter that stops fast vibrations from masquerading as slow ones (anti-alias filter). Values are stored as the raw sensor output, and the conversion factors (sensitivities) live in the set-up description.
...
|---|---|---|
| 0 (weeks 1–2) | Agree how to record; make a first trial recording (the shake-down); check that the sensors are in sensible places against a hand calculation or a computer model of the bridge | The first three or more notes of the bridge are found consistently across the sensors |
...
| 3 (version 2) | Use strain and traffic data for load-related indicators and influence lines (see glossary) | — |

## 8. How we will know it works
- How often each reference note is found per block of data.
- How much the corrected frequencies still wander, measured as the coefficient of variation (aim: less than 1%).
...
- The sensors may be too noisy below 1 Hz (one cycle per second) to see the bridge's slowest note, the fundamental.
...
- **Baseline:** the initial period in which the system learns what "normal" looks like across the range of weather and traffic. Its length is to be agreed with the owner; the plan assumes roughly three months of data before it can be signed off. No alerts are raised during it.
- **Block (window):** one stretch of data, 30–60 minutes long, processed as a unit. One file from the recorder is one block.
- **Channel:** one sensor's stream of numbers. A sensor that measures in three directions gives three channels.
- **CSV:** a plain text file of comma-separated values that any spreadsheet program can open.
...
- **Harmonic:** a signal at one fixed frequency and its multiples that does not come from the bridge: vibration from rotating machinery such as pumps or generators, or electrical interference from the mains supply (50 Hz on this site; 60 Hz in some countries). It must be recognised so that it is not mistaken for a bridge note.
...
- **pyOMA2:** a published Python software toolkit that implements OMA methods, including SSI-cov and FDD. Its licence terms for commercial use are an open question in the PRD.
...
- **SSI-cov (covariance-driven stochastic subspace identification):** the main OMA method. It fits a mathematical model of the vibrating structure to the correlations between the recorded channels and gives accurate frequencies, damping values and shapes, with uncertainty bounds.
...
- **Threshold:** the level a monitored value must cross before an alert is raised.
